<?php

return [
  's1' => '<p>Are you at least 16 on the day before elections?</p>',
  's1.1' => '<p>I am under 16 on the day before elections</p>',
  's1.2' => '<p>I am 16 or over 16 on the day before elections</p>',
  's2' => '<p>You can\'t vote: Thank you for your interest in getting involved! We are looking forward to seeing you at the next elections. In the meantime, keep being an active citizen!</p>',
  's3' => '<p>On the day of the election, which country do you want to vote from?</p>',
  's3.1' => '<p>Malta</p>',
  's3.2' => '<p>Outside Malta</p>',
  's4' => '<p>Do you have a disability or experience any limitations preventing you from reaching a polling station?</p>',
  's4.1' => '<p>Yes</p>',
  's4.2' => '<p>No</p>',
  's5' => '<p><em>For Maltese and other EU citizens:</em></p>
<p><strong>Where do I vote?</strong> You vote in a specific polling station based on your home address. This is <strong>indicated on the voting document</strong> distributed prior to election day by the police. Voters do not need to register in advance, as this is done automatically by the authorities. However, you can check if you are listed on the electoral register via the <a rel="noopener noreferrer" target="_blank" href="https://electoral.gov.mt/">website of the Electoral Commission Malta</a>, the national election authority.</p>
<p>Additional <strong>info for EU</strong> citizen voting in Maltese EU elections: You have to be registered and included in the rolling Electoral Register to be published not later than 15th April 2024. To find out how, check regularly the <a rel="noopener noreferrer" target="_blank" href="https://electoral.gov.mt/">website of the Electoral Commission Malta</a>.</p>
<p><strong>What documents should I bring?</strong> Voting document (obligatry) and ID card (recommended).</p>
<p><strong>Accesibility information:</strong> Polling stations are fully accessible. Elderly, sick, or individuals with disabilities are given priority in the queue at a specific time on polling day. Braille templates and playback listening devices are provided for visually impaired voters. Individuals with disabilities can request assistance from Assistant Electoral Commissioners to mark their ballot, with the Commissioners bound to secrecy about the votes they assist with.</p>',
  's6' => '<p><em>For Maltese and other EU citizens:</em></p>
<p><strong>Where do I vote?</strong> You vote in a specific polling station based on your home address. This is <strong>indicated on the voting document</strong> distributed prior to election day by the police. Voters do not need to register in advance, as this is done automatically by the authorities. However, you can check if you are listed on the electoral register via the <a rel="noopener noreferrer" target="_blank" href="https://electoral.gov.mt/">website of the Electoral Commission Malta</a>, the national election authority.</p>
<p>Additional <strong>info for EU</strong> citizen voting in Maltese EU elections: You have to be registered and included in the rolling Electoral Register to be published not later than 15th April 2024. To find out how, check regularly the <a rel="noopener noreferrer" target="_blank" href="https://electoral.gov.mt/">website of the Electoral Commission Malta</a>.</p>
<p><strong>What documents should I bring?</strong> Voting document (obligatry) and ID card (recommended).</p>',
  's7' => '<p>Outside Malta</p>',
  's7.1' => '<p>I am eligible to and want to vote in <strong>Maltese</strong> EU elections <strong>from another EU/non-EU country.</strong></p>',
  's7.2' => '<p>I am eligible and want to vote in EU elections <strong>for another EU member state</strong>.</p>',
  's8' => '<p>Voting in the European elections from another country is not possible.
Will you return back to Malta to vote In elections?</p>',
  's8.1' => '<p>Yes, I will go to Malta to vote in EU elections</p>',
  's8.2' => '<p>No, I will not be able to go to Malta to vote in EU elections</p>',
  's9' => '<p>Unfortunately, voting in the European elections from another country is not possible.</p>',
  's10' => '<p>Which EU country\'s elections will you be participating in as a voter?</p>',
];
